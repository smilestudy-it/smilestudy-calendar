...
export const CLASSROOMS_NAME_ACTIVE_UNIQUE_INDEX =
  'classrooms_name_active_unique';
...
/** `drizzle` partial unique index on active classroom holidays (classroomId + date) */
...
export function isD1HolidayClassroomDateUniqueViolation(
  error: unknown,
): boolean {
  const text = collectErrorTextParts(error).join(' ');
