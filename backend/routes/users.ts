...
usersApp.post('/', auth, loadUser, requireManagerOrAbove, async (c) =>{
  ...
  try {
    const auth0Result = await auth0.createAuth0User(c.env, managementToken, input.email, displayName);
    if (!auth0Result.ok) {
    ...
    passwordEmailSent = await auth0.sendAuth0PasswordSetupEmail(c.env, input.email);
    ...
    if (managementToken && auth0UserId) {
      const auth0RollbackDeleted = await auth0.deleteAuth0User(c.env, managementToken, auth0UserId);
...
// ユーザ取得(教室ごと)
usersApp.get('/:classroomId', auth, loadUser, requireManagerOrAbove, requireClassroomScope((c) => c.req.param('classroomId') ?? null), async(c) =>{
...
usersApp.delete('/:id', auth, loadUser, requireManagerOrAbove, async(c) => {
  ...
  if(actor.role === 'manager' && (target.role === 'admin' || (actor.classroomId !== target.classroomId))){
    return c.json({ message: 'forbidden' }, 403);
  }
